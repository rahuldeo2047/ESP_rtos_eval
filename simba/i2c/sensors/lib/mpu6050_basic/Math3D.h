...
inline struct Quat MulQQ(const struct Quat lhs, const struct Quat rhs)  // multiply: Quat * Quat --16 mult, 6 add, 6 sub  (266us)
{
    struct Quat a;
...
inline struct Quat MulQV(const struct Quat lhs, const struct Vec3 rhs)  // multiply: Quat * [0,vec3] --12mult, 3 add, 6 sub
{
    struct Quat a;
...
inline struct M3x3 MulMM(const struct M3x3 l, const struct M3x3 r)  // multiply: Q3x3 * Q3x3 --27mult, 18 add
{
    struct M3x3 a;
...
inline struct Quat MulQS(struct Quat a, const float b) // multiply: quat * float
{
    a.w *= b;
    a.x *= b;
    a.y *= b;
...
inline struct Quat MulSQ(const float b, const struct Quat a) // multiply: float * quat
{
    return MulQS(a, b);
}

inline struct Vec3 MulVS(struct Vec3 a, const float b) // multiply: vec3 * float
{
    a.x *= b;
    a.y *= b;
    a.z *= b;
...
inline struct Vec3 MulSV(const float b, const struct Vec3 a) // multiply: float * vec3
{
    return MulVS(a, b);
}

inline struct Quat SumQQ(struct Quat a, const struct Quat b)
{
    a.w += b.w;
    a.x += b.x;
    a.y += b.y;
...
inline struct Vec3 SumVV(struct Vec3 a, const struct Vec3 b)
{
    a.x += b.x;
    a.y += b.y;
    a.z += b.z;
...
inline struct Vec3 SubVV(struct Vec3 a, const struct Vec3 b)
{
    a.x -= b.x;
    a.y -= b.y;
    a.z -= b.z;
...
inline struct Vec3 CrossProdVV(const struct Vec3 L, const struct Vec3 R) // cross product of 3D vectors
{
    struct Vec3 a;
    a.x = L.y * R.z - L.z * R.y;
    a.y = L.z * R.x - L.x * R.z;
...
inline float DotProdQQ(const struct Quat L, const struct Quat R)
{
    return L.w * R.w + L.x * R.x + L.y * R.y + L.z * R.z;
}

inline float DotProdVV(const struct Vec3 L, const struct Vec3 R)
{
    return L.x * R.x + L.y * R.y + L.z * R.z;
}

inline struct Vec3 Vector(const float x, const float y, const float z) // 3x float to vector
{
    struct Vec3 a;
    a.x = x;
    a.y = y;
...
inline struct Quat Conj(struct Quat a)
{
    //a.w = a.w;
    a.x = -a.x;
    a.y = -a.y;
...
inline struct Quat Vector2Quat(const struct Vec3 a)    // [0,Vec3] to Quat
{
    struct Quat b;
    b.w = 0.0f;
    b.x = a.x;
...
inline struct Vec3 Quat2Vector(const struct Quat a)    // Quat to Vec3
{
    struct Vec3 b;
    b.x = a.x;
    b.y = a.y;
...
inline float MagnitudeQ(const struct Quat a)
{
    return sqrt(a.w*a.w + a.x*a.x + a.y*a.y + a.z*a.z);
}

inline float MagnitudeV(const struct Vec3 a)
{
    return sqrt(a.x*a.x + a.y*a.y + a.z*a.z);
}

inline struct Vec3 NormalizeV(const struct Vec3 a)
{
    return MulVS(a, InvSqrt(a.x*a.x + a.y*a.y + a.z*a.z));
}

inline struct Vec3 NormalizeFastV(const struct Vec3 a)
{
    return MulVS(a, InvSqrtFast(a.x*a.x + a.y*a.y + a.z*a.z));
}

inline struct Quat NormalizeQ(const struct Quat a)
{
    return MulQS(a, InvSqrt(a.x*a.x + a.y*a.y + a.z*a.z + a.w*a.w));    // 148us
    //return Mul(a, 1.0f / sqrt(a.x*a.x + a.y*a.y + a.z*a.z + a.w*a.w));  // 164us
}

inline struct Quat NormalizeFastQ(const struct Quat a)  // 120us
{
    return MulQS(a, InvSqrtFast(a.x*a.x + a.y*a.y + a.z*a.z + a.w*a.w));
}
...
inline struct Vec3 RotateMV(const struct M3x3 L, const struct Vec3 R) // vector rotated by matrix (V^ = Matrix * V -- 148us)
{
    struct Vec3 a;
...
inline struct Vec3 RotateVM(const struct Vec3 L, const struct M3x3 R) // vector rotated by matrix (V^ = V * Matrix -- 148us)
{
    struct Vec3 a;
...
inline struct Vec3 RotateVQ(const struct Vec3 v, const struct Quat q)    // Vector rotated by a Quaternion (matches V^ = V * Matrix)
{
    // v + 2*r X (r X v + q.w*v) -- https://en.wikipedia.org/wiki/Quaternions_and_spatial_rotation#Performance_comparisons
    struct Vec3 r;         // vector r is the three imaginary coefficients of quaternion q
    r.x = -q.x;    // reverse signs to change direction of rotation
    r.y = -q.y;
    r.z = -q.z;
    return SumVV(v, CrossProdVV(SumVV(r, r), SumVV(CrossProdVV(r, v), MulSV(q.w, v)))); // 296us

    //return Quat2Vector(Mul(Mul(q, v), Conj(q)));        // 460us
}

inline struct Vec3 RotateQV(const struct Quat q, const struct Vec3 v)    // Vector rotated by a Quaternion (matches V^ = Matrix * V)
{
    // v + 2*r X (r X v + q.w*v) -- https://en.wikipedia.org/wiki/Quaternions_and_spatial_rotation#Performance_comparisons
    struct Vec3 r;
    r.x = q.x;
...
inline struct Quat QuaternionVS(const struct Vec3 w, const unsigned long t)    // (angular vel vector[rad/s], time interval[us]) -- Small angle approximation
{
    float dT_2;
    struct Quat  a;
...
inline struct Quat QuaternionV(const struct Vec3 w)    // (angle vector[rad])    --Large Rotation Quaternion
{
    struct Quat a;

    float vMag = MagnitudeV(w);
...
inline void Quat2Matrix(const struct Quat q, struct M3x3 m) // Quaternion ==> Matrix
{
...
inline struct Vec3 VerticalInBody(const struct Quat q) // Quaternion ==> vertical unit vector rotated from earth to body frame
{
...
inline float atan2fast( float y, float x )
{
    ...
    if ( fabs( z ) < 1.0f )
    {
        atanf = z/(1.0f + 0.28f*z*z);
        if ( x < 0.0f )
        {
            if ( y < 0.0f ) return atanf - 3.14159265f;
            return atanf + 3.14159265f;
        }
    }
    else
    {
        atanf = 1.5707963f - z/(z*z + 0.28f);
        if ( y < 0.0f ) return atanf - 3.14159265f;
    }
    return atanf;
}
...
inline float YPR_Yaw(const struct Quat q)
{
    return atan2fast(q.y * q.z + q.w * q.x, 0.5f - (q.x * q.x + q.y * q.y));    // Yaw
}

inline float YPR_Pitch(const struct Quat q)
{
    return asin(2.0f * (q.x * q.z - q.w * q.y));;    // Pitch
}

inline float YPR_Roll(const struct Quat q)
{
...
inline float RPY_Roll(const struct Quat q)
{
    return atan2fast(q.w * q.x + q.y * q.z,  0.5f - (q.x * q.x + q.y * q.y));    // Roll
}

inline float RPY_Pitch(const struct Quat q)
{
    return asin(2.0f * (q.w * q.y - q.z * q.x));    // Pitch
}

inline float RPY_Yaw(const struct Quat q)
{
    return atan2fast(q.w * q.z + q.x * q.y,  0.5f - (q.y * q.y  + q.z * q.z));    // Yaw
}

inline struct Vec3 RollPitchYaw(const struct Quat q)
{
    struct Vec3 ypr;
...
// void displayV(const struct Vec3 v)
// {
//     #ifdef SERIAL_PORT
//         String outputBuffer;
//
...
// }
//
// void displayQ(const struct Quat q)
// {
//     #ifdef SERIAL_PORT
//         String outputBuffer;
//
...
// }
//
// void displayM(const struct M3x3 m)
// {
//     #ifdef SERIAL_PORT
//         String outputBuffer;
//
